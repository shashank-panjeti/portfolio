  ...
  export function DynamicProjectDetail({ project }: DynamicProjectDetailProps) {
    ...
    const hasCaseStudyNav =
      caseStudySections.length > 0 && project.category !== "interior" && project.category !== "3d"

    // Build the side-nav items (prepend "Project" entry)
    const navItems =
      hasCaseStudyNav
        ? caseStudySections.map(s => ({
              id: s.id,
              navLabel: s.navLabel,
              navFullLabel: s.navFullLabel,
            }))
        : []
    ...
    useEffect(() => {
      ...
      // Observe top "project" anchor
      const projectEl = document.getElementById("project")
      if (projectEl) observer.observe(projectEl)

      // Observe each case study section
    ...
    const scrollToSection = (id: string) => {
      const targetId = id === "project" ? "project" : id
    ...
    return (
      <div className="space-y-16">
        {heroSection ? (
          ...
          // Fallback anchor if no hero
          <div id="project" className="h-0" />
        )}
        ...
        {hasCaseStudyNav ? (
          <div ref={contentStartRef} className="max-w-[1400px] mx-auto">
            <div
              className={` max-w-50
                hidden md:block sticky transition-all duration-300 z-40
                ${isNavCentered ? "top-20 opacity-100 translate-y-0" : "top-20 opacity-0 -translate-y-2"}
              `}
              aria-hidden={!isNavCentered}
            >
              <h2 className="text-lg lg:text-xl font-medium tracking-wide text-foreground/80">
                {project.title}
              </h2>
              <div className="mt-2 h-px bg-border/75 max-w-[1400px] mx-auto" />
            </div>         
            <div className="flex gap-4 items-start">
                {/* <div
                  className={`hidden md:block w-56 shrink-0 absolute transition-all duration-300 top-20 left-15
                    overflow-hidden 
                    ${isNavCentered ? "max-h-8 mb-4 opacity-100 translate-y-0" : "max-h-0 mb-0 opacity-0 -translate-y-1"}
                  `}
                  aria-hidden={!isNavCentered}
                >
                  <h2 className="text-md md:text-lg lg:text-xl font-medium tracking-wide text-foreground/70 line-clamp-1 ">
                    {project.title}
                  </h2>
